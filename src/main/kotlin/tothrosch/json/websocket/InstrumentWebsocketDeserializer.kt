/*
package tothrosch.json.websocket
...
object InstrumentWebsocketDeserializer {

	class Bitmex: StdDeserializer<Instruments> {
		...
		override fun deserialize(p: JsonParser?, ctxt: DeserializationContext?): Instruments {
			val jacksonArrayNode: ArrayNode =
				p?.codec?.readTree(p) ?: throw JsonException("cannot deserialize json because $p is null")
			val globalInstruments: MutableSet<Instrument> = mutableSetOf()
			for (node: JsonNode in jacksonArrayNode) {
				if (node["state"].asText() != "Open") continue
				if (node["typ"].asText() == "FFWCSX") {
					globalInstruments.add(p.codec.treeToValue(node, Instrument.Bitmex.Swap::class.java))
				} else if (node["typ"].asText() == "FFCCSX") {
					globalInstruments.add(p.codec.treeToValue(node, Instrument.Bitmex.Future::class.java))
				}
			}

			return Instruments(globalInstruments)
		}
	}
}*/
