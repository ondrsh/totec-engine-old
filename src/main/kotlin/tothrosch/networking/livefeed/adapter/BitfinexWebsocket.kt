...
object BitfinexWebsocket: WebsocketClient(Exchange.BitfinexExchange) {
	
	override val address: String = "wss://api.bitfinex.com/ws/2"
	...
	override suspend fun processMessage(msg: String) {
		...
		if (node.isArray) {
			...
			val instrument: Instrument = channelToInst[channel] ?: throw LiveFeedException("Bitfinex websocket error - couldn't get instrument from chanId from message $msg")
			val type: Int = channelToType[channel] ?: throw LiveFeedException("Bitfinex websocket error -couldn't get channeltype from message $msg")
			if (node[1].asText() == "hb") {
				return
			}
			when (type) {
				...
				2    -> {
					val bookArray = node[1] as ArrayNode
					if (bookArray[0] is ArrayNode) {
						processFullBook(bookArray, instrument)
					} else {
						processBookMessage(bookArray, instrument)
					}
				}
				else -> throw LiveFeedException("Bitfinex Websocket error - couldn't determine channel type of message $msg")
			}
			
		}
	}
	
	fun addChannel(node: JsonNode) {
		...
		channelToInst[chanId] = instrument
		channelToType[chanId] = if (type == "trades") 1 else if (type == "book") 2 else throw LiveFeedException("Bitfinex Websocket - couldn't determine channel type of message ${node.asText()}")
	}
	...
	suspend fun processFullBook(jsonArray: ArrayNode, instrument: Instrument) {
		...
		val bidsMap: HashMap<String, BookEntry> = HashMap(jsonArray.filter { it.get(2).asDouble() > 0 }.associate {
			val price = it[0].asDouble()
			val amount = it[2].asDouble()
			val id = price.toString()
			id to BookEntry(price, amount, id)
		})
		
		
		val asksMap: HashMap<String, BookEntry> = HashMap(jsonArray.filter { it.get(2).asDouble() < 0 }.associate {
			val price = it[0].asDouble()
			val amount = -1 * it[2].asDouble()
			val id = price.toString()
			id to BookEntry(price, amount, id)
		})
	...
	suspend fun processBookMessage(jsonArray: ArrayNode, instrument: Instrument) {
		val price: Double = jsonArray.get(0).asDouble()
		val numberOfOrders: Int = jsonArray.get(1).asInt()
		val amountMaybeNegative: Double = jsonArray.get(2).asDouble()
		val amount: Double = abs(amountMaybeNegative)
		val side: Side = when (sign(amountMaybeNegative)) {
			1.0  -> Side.BUY
			-1.0 -> Side.SELL
		...
		if (numberOfOrders == 0) instrument.channel.send(Message(BookOperations(listOf(BookOperation.Delete(side, bookEntry)))))
	...
	suspend fun processTradeMessage(jsonArray: ArrayNode, instrument: Instrument) {
		...
		val tradeArray = jsonArray.get(2) as ArrayNode
		val price: Double = tradeArray.get(3).asDouble()
		val amountMaybeNegative: Double = tradeArray.get(2).asDouble()
		...
		instrument.channel.send(Message(Trades(listOf(Trade(price = price,
		                                                    amount = amount,
		                                                    initiatingSide = side)))))
		// instrument.bookMessageHandler.send(Message(BookOperations(listOf(BookOperation.Trade (if (side == Side.BUY) Side.SELL else Side.BUY, BookEntry(price, amount, price.toString()))))))
	}
